...
export class TaskManagerJanitor {
  ...
  constructor(knex: Knex, waitBetweenRuns: Duration, logger: Logger) {
    this.knex = knex;
  ...
  async start() {
    while (!this.cancelToken.isCancelled) {
      ...
      await Promise.race([
        new Promise(resolve =>
          setTimeout(resolve, this.waitBetweenRuns.as('milliseconds')),
        ),
  ...
  private async runOnce() {
    ...
    const tasksQuery = this.knex<DbTasksRow>('backstage_backend_common__tasks')

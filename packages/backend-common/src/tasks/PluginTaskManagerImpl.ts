...
import { DbMutexesRow, DbTasksRow } from '../database/tables';
...
export class PluginTaskManagerImpl implements PluginTaskManager {
  ...
  > {
    ...
    async function release() {
      try {
        await knex<DbMutexesRow>('backstage_backend_common__mutexes')
          .where('id', '=', id)
    ...
    const stolen = await knex<DbMutexesRow>('backstage_backend_common__mutexes')
      .where('id', '=', id)
    ...
    try {
      await knex<DbMutexesRow>('backstage_backend_common__mutexes').insert({
        id,
        current_lock_ticket: ticket,
        current_lock_acquired_at: knex.fn.now(),
        current_lock_expires_at: nowPlus(options.timeout, knex),
